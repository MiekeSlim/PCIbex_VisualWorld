    ...
    function calibrate(resolve, element, threshold, remainingAttempts){
        ...
        let startCalculation = ()=>{
            ...
            calibrationDiv.append($("<button>+</button>").css({
                ...
                calibrationDiv.find('button').remove();
                calibrationDiv.append($("<div>").html("<p>Look<br>here!</p>").css({position: 'absolute', top: "calc(50vh - 1.5vw)", left: "48.5vw", width: "3vw", height: "3vw", "font-size": "1vw"}));
                storePoints = true;
                setTimeout(()=>{
                    ...
                    if (threshold && Number(threshold)>0 && precision < threshold && remainingAttempts != 1){
                        calibrated = false; 
                        showTracker(true);
                        calibrationDiv.append($("<div>").html("<p>    <br>    </p>").css({position: 'absolute', top: "calc(50vh - 1.5vw)", left: "48.5vw", width: "3vw", height: "3vw", "font-size": "1vw", "background-color": "white"}));
